...
async function getData(type, value) {
    try {
        ...
        const res = await fetch(`http://localhost:8080/customer/find-by-search/${type}?value=${value}`, {
            method: "GET",
            headers: {
...
async function removeRow(id) {
    ...
    try {
        ...
        const res = await fetch(`http://localhost:8080/customer/delete?id=${id}`, {
            method: 'DELETE',
            headers: {
...
async function addUser(user) {
    try {
        const token = sessionStorage.getItem("token");
        const res = await fetch(`http://localhost:8080/customer/add`, {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${token}`,
                'Content-Type': 'application/json',
...
async function editUser(userId) {
    const token = sessionStorage.getItem("token");
    const res = await fetch(`http://localhost:8080/customer/find-by-id?id=${userId}`, {
        method: "GET",
        headers: {
            "Authorization": `Bearer ${token}`,
            'Content-Type': 'application/json',
...
updateUserBtn.addEventListener('click', (e) => {
    e.preventDefault();
    let user = {
        "first_Name": submittedForm.first.value,
        "last_Name": submittedForm.last.value,
...
async function updateUser(user, id) {
    try {
        const token = sessionStorage.getItem("token");
        const res = await fetch(`http://localhost:8080/customer/update?id=${id}`, {
            method: "PUT",
            headers: {
                "Authorization": `Bearer ${token}`,
                'Content-Type': 'application/json',
...
async function getCustomerList(data) {
    ...
    try {
        ...
        arr.forEach(async (ele) => {
            ...
            const userCheck = await fetch(`http://localhost:8080/customer/find-by-search/phone?value=${user.phone}`, {
                method: "GET",
                headers: {
